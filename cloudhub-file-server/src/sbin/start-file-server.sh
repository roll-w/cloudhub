...
if [ "$CLOUDHUB_FILE_HOME" = '' ]; then
  echo "Failed: Not set CLOUDHUB_FILE_HOME variable in the env, program exit with code 2."
  exit 2
fi

CONF_DIR=$CLOUDHUB_FILE_HOME/conf

cd "$CLOUDHUB_FILE_HOME" || echo "Failed: Not found the path $CLOUDHUB_FILE_HOME, program exit with code 3." exit 3
