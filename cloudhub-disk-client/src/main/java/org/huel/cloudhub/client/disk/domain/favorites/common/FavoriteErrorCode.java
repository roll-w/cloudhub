 ...
 * @author RollW
 */
public enum FavoriteErrorCode implements ErrorCode, ErrorCodeFinder, ErrorCodeMessageProvider {
    ERROR_FAVORITE("B3700", 400),
    ERROR_FAVORITE_NOT_FOUND("B3701", 404),
    ERROR_FAVORITE_EXISTED("B3702", 400),
    ERROR_FAVORITE_NAME_NON_COMPLIANCE("B3703", 400),
    ;

    private final String value;
    private final int status;

    FavoriteErrorCode(String value, int status) {
        this.value = value;
        this.status = status;
    }
    ...
    public String toString() {
        return "FavoriteError: %s, code: %s".formatted(name(), getCode());
    }
